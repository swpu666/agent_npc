...
### BC-5（已随功能撤下而消失）错误消息有误导性：可能是「知识缺口」先挂
...
- **后续（2026-09-23）**：使用者反馈"知识缺口聚合范围宽了、根本不准"，已把这整张卡片从抽屉里撤下，
  `openMemory()` 现在只请求本会话画像。因此这条 badcase 的表现形式（"可能是第二段挂了"）不再存在；
  接口 `GET /api/knowledge-gaps` 仍保留，供需要时按意图过滤后使用。
